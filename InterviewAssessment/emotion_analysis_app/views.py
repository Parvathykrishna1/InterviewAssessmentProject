...
def emotion_analysis(request):
    ...
    while time.time() < end_time:
        ...
        for (x, y, w, h) in faces_detected:
            ...
            if confidence_score >= min_confidence and capture_count < max_captures:
                ...
                cv2.imwrite(image_filename, test_img)
                confidence_scores.append(confidence_score)
                captured_image = CapturedImage(
                    capturedimage=image_filename,
    ...
    return render(request, 'emotion_analysis.html',
                  {'captured_images': captured_images, 'average_confidence_percentage': average_confidence_percentage})